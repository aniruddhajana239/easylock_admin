...
const [appliedToDate, setAppliedToDate] = useState('');
  ...
  useEffect(() => {
    setDevices([])
    setTotalCount(0)
    return ()=> dispatch(deviceActions.reset())
  ...
  const categories = ["All", "Locked", "Unlocked", "Removed"];
    ...
    <tbody className="divide-y divide-slate-200">
      {devices.map((item, index) => (
        <tr
          ...
          onClick={() => navigate(`/device-lock/devices/details/${item.deviceUniqueID}`)}
        >
          <td className="py-4 px-4">
            <div className="flex items-center">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-slate-500">{item?.imeiNo}</span>
              </div>
            </div>
          </td>
          <td className="py-4 px-4">
          ...
          <td className="py-4 px-4">
            <span className="font-medium text-slate-900">
              ₹{!isNaN(Number(item?.devicePrice)?.toFixed(2)) ? Number(item?.devicePrice)?.toFixed(2) : "0.00"}
            </span>
          </td>
          <td className="py-4 px-4">
          ...
          <td className="py-4 px-4">
            <span className="text-slate-600">
              {formatEnrollmentDate(item?.enrollmentDate)}
            </span>
          </td>
        </tr>
      ))}
  ...
  useEffect(() => {
  fetchData();
}, [page, rowsPerPage, debouncedSearchText, isSearching, appliedFromDate, appliedToDate]);
            ...
            <div className="mb-6 space-y-4">
              <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                ...
                <div className="w-full lg:w-96">
                  <label htmlFor="search" className="block text-sm font-medium text-slate-600 mb-1.5">
                    Search Devices
                  </label>
                ...
                {/* <div className="flex flex-col sm:flex-row gap-4">
                  <div>
                    <label htmlFor="fromDate" className="block text-sm font-medium text-slate-600 mb-1.5">
                      From Date
                    </label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label htmlFor="toDate" className="block text-sm font-medium text-slate-600 mb-1.5">
                      To Date
                    </label>
                    <input
                      type="date"
...
export default DeviceDetailsByIMEI;