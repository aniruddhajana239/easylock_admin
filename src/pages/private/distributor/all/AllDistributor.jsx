...
import { FaSearch, FaEnvelope, FaPhone, FaMapMarkerAlt, FaKey, FaStore } from "react-icons/fa";
...
const AllDistributor = () => {
  ...
  const onCloseAddModal = () => {
    setModalOpen(false);
    dispatch(
        distributorActions.getAll({ page: page + 1, limit: rowsPerPage })
      );
  };
  ...
  useEffect(() => {
    if (
      ...
      setDistributorData(selector?.data?.data?.rows);
  ...
  const handleConfirmStatusChange = async () => {
    if (selectedDistributor) {
      ...
      try {
        const response = await DistributorApi.updateAccountStatus({
    ...
    <tbody className="divide-y divide-slate-200">
      {distributorData.map((item, index) => (
        <tr 
          ...
          onClick={() => {
            navigate(`/device-lock/distributors/all/details/${item.id}`);
          }}
        >
    ...
    <div className="min-h-screen py-4">
      <div className="w-full">
          ...
          <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Manage and oversee all distributor accounts
              </p>
            </div>
            <Button
              variant="contained"
        ...
        >
          ...
          ) : Array.isArray(distributorData) && distributorData?.length > 0 ? (
            <Table
